...
```mermaid
graph TD;
    PI[Perceptual Interface] --> TSL[Temporal Sequencing Layer];
    TSL --> SeqMap[Sequence Map];
    TSL --> CausalProj[Causal Projection];
    TSL --> Envelope[Future-State Envelope];
    TSL --> Timing[Timing Constraints];
    TSL --> Avoid[Collision-Avoidance Vectors];
```
...
```mermaid
graph TD;
    PI[Perceptual Interface] --> TSL[TSL: Temporal Sequencing Layer];
    TSL --> SME[SME: Structural Mapping Engine];
    SME --> NSCL[NSCL: Negative-Space Cognition Layer];
    NSCL --> Prune[Branch Pruning & Timeline Suppression];
    NSCL --> CleanSeq[Contradiction-Free Sequences];
    NSCL --> Downstream[Downstream Simplicity];
```
...
```mermaid
graph LR;
    SME[Structural Mapping Engine] --> NSCL[Negative-Space Cognition Layer];
    IC[Invariant Core] --> NSCL;
    TSL[Temporal Sequencing Layer] --> NSCL;
    NSCL --> Prune[Pruned Branches];
    NSCL --> Quiet[Minimal Visible Behavior];
    NSCL --> CleanSeq[Clean Sequences];
    NSCL --> Simplicity[Downstream Simplicity];
```
